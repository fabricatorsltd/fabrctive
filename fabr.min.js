...
const iconElement=this.iconDOMStructure.cloneNode(true);iconElement.previousIcon="{{icon}}";iconElement.changeIcon=(icon)=>{iconElement.innerHTML=iconElement.innerHTML.replace(iconElement.previousIcon,icon);iconElement.previousIcon=icon;};iconElement.changeIcon(icon);return iconElement;}};fbr.FabrHelperLocalStorage=class extends fbr.FabrHelper{constructor(){super();this.helperName="LocalStorage";}
...
return false;}};fbr.FabrHelperToast=class extends fbr.FabrHelper{constructor(){super();this.helperName="Toast";}
...
setTimeout(()=>{toast.remove();},duration);}};fbr.FabrHelperWatcher=class extends fbr.FabrHelper{constructor(){super();this.helperName="Watcher";this.watchers={};}
...
return result;}};fbr.FabrCoreComponent=class extends fbr.FabrCore{constructor(){super();this.componentUID=Math.random().toString(36).substr(2,9);this.componentName="Generic";this.componentType="generic";this.componentStyleClass="fabr-component";this.selector="";this.eventMap={};this.elements=[];this.renderedElements=[];}
...
this.elements=document.querySelectorAll(this.selector);this.elements.forEach((element)=>{if(this.renderedElements.includes(element)){return;}
element.classList.add(this.componentStyleClass);this.renderedElements.push(element);});}
...
addInternalEventListener(element,event,fn,reference=null,...args){const listener=element.addEventListener(event,(e)=>{if(reference){this[fn](e,reference,...args);}else{this[fn](e,...args);}});return listener;}
removeInternalEventListener(element,event,listener){element.removeEventListener(event,listener);}
#renderElements(){if(this.elements.length===0){return;}
this.elements.forEach((element)=>{if(this.render){this.render(element);}});}};fbr.FabrAnimator=class extends fbr.FabrCoreComponent{constructor(){super();this.componentName="FabrAnimator";this.componentStyleClass="fabr-animator";this.selector="[fabr-animator]";this.eventMap={click:"onAnim",mouseover:"onAnim",mouseout:"onAnim",};this.animations={};this.animateHelper=new fbr.FabrHelperAnimate();this.animateHelper.init(this);}
...
this.animateHelper.animate(element,animationType);}};fbr.FabrCounter=class extends fbr.FabrCoreComponent{constructor(){super();this.componentName="FabrCounter";this.componentStyleClass="fabr-counter";this.selector="[fabr-counter]";this.eventMap={click:"onClick",};this.counters={};}
...
render(selector){const wrapper=document.createElement("div");wrapper.classList.add("fabr-selector-wrapper");const searchWrapper=document.createElement("div");searchWrapper.classList.add("fabr-selector-search-wrapper");wrapper.appendChild(searchWrapper);const options=selector.querySelectorAll("option");const dropdown=document.createElement("div");dropdown.classList.add("fabr-selector-dropdown");selector.parentNode.insertBefore(wrapper,selector);wrapper.appendChild(selector);const input=document.createElement("input");input.classList.add("fabr-selector-input");input.type="text";input.placeholder="Search...";searchWrapper.appendChild(input);const icon=this.iconHelper.new("arrow_drop_down");icon.classList.add("fabr-selector-icon");searchWrapper.appendChild(icon);const list=document.createElement("ul");list.classList.add("fabr-selector-list");dropdown.appendChild(list);options.forEach((option)=>{const listItem=document.createElement("li");listItem.classList.add("fabr-selector-item");listItem.dataset.value=option.value;listItem.textContent=option.textContent;list.appendChild(listItem);});this.addInternalEventListener(list,"keydown","onKeyNavigationEvent",null,selector,input);this.addInternalEventListener(input,"keydown","onKeyNavigationEvent",null,selector,input);this.addInternalEventListener(input,"input","filterOptionsEvent",null,list);this.addInternalEventListener(searchWrapper,"click","showDropdownEvent",null,list);this.addInternalEventListener(list,"click","selectOptionEvent",null,selector,input,list);this.addInternalEventListener(document,"click","hideDropdownEvent",null,wrapper,list);wrapper.appendChild(dropdown);selector.style.display="none";}
...
onHeaderClick(event,expander){const wrapper=expander.activeElement;const content=wrapper.querySelector(".fabr-expander-content");const icon=wrapper.querySelector(".fabr-expander-icon");if(content.classList.contains("fabr-expander-content-open")){this.animateHelper.fadeOut(content,false,300);content.classList.remove("fabr-expander-content-open");icon.textContent="keyboard_arrow_down";return;}
this.animateHelper.fadeIn(content,300);content.classList.add("fabr-expander-content-open");icon.textContent="keyboard_arrow_up";}};fbr.Fabr=class extends fbr.FabrCore{constructor(){super();this.version="0.1.0";console.log(`%cFabr[${this.version}]initializing...`,"color: #00f");this.components={form:new fbr.FabrForm(),link:new fbr.FabrLink(),list:new fbr.FabrList(),counter:new fbr.FabrCounter(),expander:new fbr.FabrExpander(),tooltip:new fbr.FabrTooltip(),notebook:new fbr.FabrNotebook(),table:new fbr.FabrTable(),animator:new fbr.FabrAnimator(),snippet:new fbr.FabrSnippet(),selector:new fbr.FabrSelector(),};this.init();console.log("%cFabr initialized","color: #00f");}
